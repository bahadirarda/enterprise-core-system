...
export class SharedAuthManager {
  ...
  async refreshSessionIfNeeded(): Promise<boolean> {
    ...
    if (this.isUserActive() && this.needsRefresh(session)) {
      try {
        console.log('🔄 SharedAuth: Attempting to refresh session...')
        console.log('🔄 SharedAuth: Using refresh token:', session.refresh_token?.substring(0, 20) + '...')
        ...
        if (error) {
          console.error('❌ SharedAuth: Refresh error:', error)
          console.error('❌ SharedAuth: Error details:', {
        ...
        console.log('🔄 SharedAuth: Refresh response:', data)
        ...
        if (data.session) {
          ...
          console.log('✅ SharedAuth: Session refreshed successfully')
          return true
        } else {
          console.error('❌ SharedAuth: No session in refresh response')
          return false
        }
      } catch (error) {
        console.error('❌ SharedAuth: Error refreshing session:', error)
        console.error('❌ SharedAuth: Full error object:', error)
        this.clearSharedSession()
        return false
      }
    }
  ...
  async checkAuthStatus(): Promise<{ isAuthenticated: boolean; user: User | null }> {
    try {
      ...
      const session = this.getSharedSession()
      console.log('🔍 SharedAuth: Retrieved session:', session ? 'Found' : 'Not found')
      
      if (!session) {
        console.log('❌ SharedAuth: No session found')
        return { isAuthenticated: false, user: null }
      }

      console.log('🔍 SharedAuth: Session expires at:', new Date(session.expires_at).toLocaleString())
      console.log('🔍 SharedAuth: Current time:', new Date().toLocaleString())

      // Refresh session if needed
      console.log('🔍 SharedAuth: Checking if refresh needed...')
      const refreshed = await this.refreshSessionIfNeeded()
      console.log('🔍 SharedAuth: Refresh result:', refreshed)
      
      if (!refreshed) {
        console.log('❌ SharedAuth: Failed to refresh session')
        return { isAuthenticated: false, user: null }
      }

      const currentSession = this.getSharedSession()
      console.log('✅ SharedAuth: Authentication successful')
      return { 
        isAuthenticated: true, 
        user: currentSession?.user || null 
      }
    } catch (error) {
      console.error('❌ SharedAuth: Error in checkAuthStatus:', error)
      return { isAuthenticated: false, user: null }
    }
  }
